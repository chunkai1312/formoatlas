...
系統 SHALL 使用 GitHub Copilot SDK 產生未快取的每日晴雨表分析，且 SHALL 一律透過 `COPILOT_CLI_URL` 連線至 Copilot CLI headless server。

#### Scenario: 已設定 Headless CLI URL
- **WHEN** 請求未快取的晴雨表分析，且 API process 可讀取 `COPILOT_CLI_URL`
- **THEN** 系統 SHALL 透過 GitHub Copilot SDK 連線至該 headless CLI server 產生分析，而不是由 API process 自行啟動 CLI
