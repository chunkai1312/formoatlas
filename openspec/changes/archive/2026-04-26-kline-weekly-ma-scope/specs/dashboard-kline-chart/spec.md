...
- **THEN** 系統 SHALL 顯示 candlestick K 棒與成交量 bar（疊於主圖下方），日K模式顯示 MA5/MA10/MA20/MA60/MA120/MA240，週K模式顯示 MA5/MA10/MA20/MA60

#### Scenario: MA info bar 常駐顯示
- **WHEN** K 線圖渲染完成
- **THEN** 圖表標題列下方 SHALL 顯示一排 MA info bar，列出當前時間粒度實際支援的 MA 值，每個 MA 值以對應顏色標示（MA5 琥珀黃、MA10 橙、MA20 淺紫、MA60 青藍、MA120 粉紅、MA240 黃綠；刻意避開 K 棒紅/綠以防撞色）；日K模式預設顯示 MA5～MA240 的最後一根 K 棒值，週K模式預設顯示 MA5/MA10/MA20/MA60 的最後一根 K 棒值

#### Scenario: MA info bar 隨 hover 更新
- **WHEN** 用戶 hover K 線圖任意 K 棒
- **THEN** MA info bar SHALL 即時更新為該 K 棒日期對應的已支援 MA 值；滑鼠離開圖表時 SHALL 自動恢復為最後一根 K 棒的 MA 值
...
#### Scenario: 日K 切換至週K
- **WHEN** 用戶點擊 `[週K]` 按鈕
- **THEN** 系統 SHALL 直接以現有近 5Y 日K快取資料聚合為週K後依 localRange 顯示，不發送新的 API 請求；若目前 localRange 為 `1M`（週K模式不支援），SHALL 自動 fallback 為 `3M`

#### Scenario: 週K 切換至日K
- **WHEN** 用戶點擊 `[日K]` 按鈕
- **THEN** 系統 SHALL 直接以現有近 5Y 日K快取資料依目前 localRange 顯示，不發送新的 API 請求（若 localRange 為 `2Y`，SHALL fallback 為 `1Y`）

#### Scenario: 日期改變觸發重新 fetch
- **WHEN** 用戶透過 Toolbar 切換日期
- **THEN** 系統 SHALL 以新 endDate 重新請求對應視窗資料（日K 5Y，週K 5Y），並依目前 localRange 重新 slice 顯示

#### Scenario: 日K 與週K 共用同一份快取
- **WHEN** 用戶在同一 symbol 與 endDate 下切換 `[日K]` 與 `[週K]`
- **THEN** 系統 SHALL 共用同一份近 5Y 日K快取資料，不因時間粒度切換而重複發送 API 請求

#### Scenario: 日K 長天期均線在 1Y 視窗完整呈現
- **WHEN** K 線圖處於日K模式且用戶選擇 `[1Y]`
- **THEN** 系統 SHALL 透過近 5Y 歷史資料，讓 MA60、MA120、MA240 在 1Y 可見範圍內不因 fetch 視窗過短而於前段缺線
...
#### Scenario: 週K 限縮長天期 MA 顯示
- **WHEN** K 線圖處於週K模式
- **THEN** 系統 SHALL 不顯示 MA120 與 MA240 的線段與 info bar 數值

#### Scenario: 週K MA60 在 2Y 視窗完整呈現
- **WHEN** K 線圖處於週K模式且用戶選擇 `[2Y]`
- **THEN** 系統 SHALL 透過近 5Y 歷史資料，讓 MA60 在 2Y 可見範圍內不因 fetch 視窗過短而於前段缺線
