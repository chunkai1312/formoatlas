...
### Requirement: 刪除使用者對話
系統 SHALL 允許已登入使用者刪除自己擁有的市場研究對話及其訊息。刪除含有 `copilotSessionId` 的 conversation 時，系統 SHALL 對 app-owned Copilot runtime session state 進行盡力清除。
...
- **THEN** 系統 SHALL 透過共用 Copilot runtime 嘗試刪除對應的 app-owned Copilot session state
- **AND** client MUST NOT 直接指定或操作 Copilot session id

#### Scenario: Copilot session cleanup 失敗
- **WHEN** app-owned conversation/messages 刪除成功，但 Copilot session cleanup 失敗
- **THEN** 系統 SHALL 記錄 cleanup 失敗
- **AND** 系統 SHALL NOT 還原已刪除的 app-owned conversation 或 messages
